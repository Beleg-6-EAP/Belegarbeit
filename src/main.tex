...
Frühere Architekturmuster, wie die monolithische Architektur, zeichneten sich durch eine
enge Kopplung der Funktionalitäten aus. Im Gegensatz dazu legt die Service-orientierte
Architektur (SOA) einen starken Fokus auf die Wiederverwendbarkeit von Funktionalitäten,
indem sie auf die Nutzung von Diensten (engl. \textit{services}) setzt.

Eine Untersuchung der wissenschaftlichen Literatur zeigt, dass SOA in unterschiedlichen
Kontexten verschieden definiert wird. Dennoch bleibt das Konzept der Dienste ein zentraler
Bestandteil dieser Architektur.

Ein Service in einer SOA ist lose gekoppelt, typischerweise grob granuliert und zeichnet
sich durch mehrere zentrale Eigenschaften aus. Seine Auffindbarkeit wird durch eine
Registry sichergestellt, während der Zugriff über einen Namen oder einen Endpoint
mittels webbasierter Technologien erfolgt. Die Verfügbarkeit einer publizierten
Spezifikation definiert wie die Interaktion mit dem Dienst erfolgt. Zudem kann der
Dienst zur Laufzeit dynamisch lokalisiert sowie genutzt werden.
\cite[16]{soa2}\cite[19]{soa4}
...
Die Registry dient als Sammlung verfügbarer Dienste und deren Spezifikationen.
Provider registrieren ihre Dienste in der Registry, während Consumer die
Möglichkeit haben, diese Dienste dynamisch zur Laufzeit zu entdecken \cite[23-24]{soa4}.
In der Dienstspezifikation wird beschrieben, wie ein Consumer mit dem Provider interagieren
kann. Dies umfasst unter anderem die Definition der Anfrage und der Antwort.
Zudem können weitere Informationen, wie etwa die Bedingungen für die Nutzung des Dienstes,
in der Spezifikation enthalten sein \cite[26]{soa4}.

Der Service Bus, insbesondere ein Enterprise Service Bus (ESB), stellt eine als Best Practice
Vorgehensweise zur Implementierung von SOA dar.
Hierbei handelt es sich um einen Messaging-Bus, dessen Funktion in der Orchestrierung
sämtlicher Aspekte der Kommunikation zwischen Consumer und Provider besteht.
...
und die vollständige Implementierung des E-Commerce-Beispiels kann bei
GitHub\footnote{https://github.com/Beleg-6-EAP/demo-soa-ecommerce} ebenfalls eingesehen werden.
